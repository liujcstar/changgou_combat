...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Transactional
    public Result addLoveSku(String username, String id) {
        //将数据保存到数据库
        Collection collection=new Collection();
        collection.setUsername(username);

        collection.setSkuId(id);
        Collection collectionInDB = collectionMapper.selectByPrimaryKey(collection);
        if (collectionInDB!=null){
            return new Result(true, StatusCode.OK,"您已收藏,请勿重复收藏");
        }
    ...
    public List<Sku> myCollection(String username) {
        ...
        for (Collection collection : collections) {
            //遍历获取商品id用于获取商品信息
            Sku sku = skuFeign.findById(collection.getSkuId()).getData();
            list.add(sku);
        }
    ...
    @Transactional
    @Override
    public void addFootMark(String username, String id) {



        //添加数据到数据库
        Footmark footmark=new Footmark();
        footmark.setSkuId(id);
        footmark.setUsername(username);
        //判断数据库是否存在避免重复添加
        Footmark foot = footmarkMapper.selectByPrimaryKey(footmark);
        if (foot!=null){
            return;
        }
    ...
    public List<Sku> myFootMark(String username) {
        ...
        for (Footmark footmark : footmarkList) {
            Sku sku = skuFeign.findById(footmark.getSkuId()).getData();
            //将商品信息加入集合返回
            skuList.add(sku);
